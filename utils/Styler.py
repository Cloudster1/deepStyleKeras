...
def style(base_img=None, style=None, size=512, fun_evals=100, weights=None, pooling='max', tv_weight=1e-3, style_weight=1e4, content_weight=5e0):
	...
	if pooling == 'max':
		if weights is not None:
			model = vgg19.VGG19(input_tensor=input_tensor, weights=weights,include_top=False)
		else:
	...
	def style(base_image_path):
		...
		for i in range(1):
			...
			x, min_val, info = fmin_l_bfgs_b(evaluator.loss, x.flatten(),
			                                 fprime=evaluator.grads, maxfun=fun_evals)
